...
def daily_readme(birthday):
    ...
    return '{} {}, {} {}, {} {}{}'.format(
        ...
        ' 🎂jk' if (diff.months == 0 and diff.days == 0) else '')
...
def force_close_file(data, cache_comment):
    ...
    """
    filename = 'AndrewKillion/cache/'+hashlib.sha256(USER_NAME.encode('utf-8')).hexdigest()+'.txt'
    with open(filename, 'w') as f:
        f.writelines(cache_comment)
        f.writelines(data)
...
def stars_counter(data):
    ...
    total_stars = 0
    for edge in data:
        repo = edge['node'] if edge else None
...
def get_title_length(element_id):
    """
    Finds the title element associated with an element_id and returns its text length.
    """
    tree = etree.parse('dark_mode.svg')
    root = tree.getroot()
    title_id = f"{element_id}_title"

    title_element = root.find(f".//*[@id='{title_id}']")

    if title_element is None:  # for +/- line which is nested differently
        print(f"Title element, {element_id} is None")
        title_element = root.find(f".//*[@id='{element_id}']")
    
    if title_element is not None:
        text_content = "".join(title_element.itertext())
        return len(text_content)
    return 0

def get_data_length(element_id):
    """
    Finds the title element associated with an element_id and returns its text length.
    """
    tree = etree.parse('dark_mode.svg')
    root = tree.getroot()
    title_id = f"{element_id}_output"
...
def commit_counter(comment_size):
    ...
    """
    total_commits = 0
    filename = 'AndrewKillion/cache/'+hashlib.sha256(USER_NAME.encode('utf-8')).hexdigest()+'.txt' # Use the same filename as cache_builder
    with open(filename, 'r') as f:
        data = f.readlines()
    cache_comment = data[:comment_size] # save the comment block
    data = data[comment_size:] # remove those lines
...
if __name__ == '__main__':
    """
    # Andrew Killion (AndrewKillion), 2022-2025
    # This file needs to be updated and pushed to github to keep the stats up-to-date
    # """


    #tree.write('dark_mode.svg', encoding='utf-8', xml_declaration=True)
    ...
    svg_overwrite('dark_mode.svg', age_data, commit_data, star_data, repo_data, contrib_data, follower_data, total_loc[:-1], bio_length, github_1_length, github_2_length)
    svg_overwrite('light_mode.svg', age_data, commit_data, star_data, repo_data, contrib_data, follower_data, total_loc[:-1], bio_length, github_1_length, github_2_length)
    ...
    # tree = etree.parse('dark_mode.svg')
    # root = tree.getroot()
    # data_output = get_data_length('repo_data')
    # print(data_output)